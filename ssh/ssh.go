...
func SshScan2(ScanType string, Target string, result mode.Result) {

Loop:
	for _, u := range dic.UserDic() {
		for _, p := range dic.PassDic() {
			//fmt.Println("Check... " + Target + " " + u + " " + p)
			datamap := map[string]string{"flag": "checking", "target": Target, "port": "22", "user": u, "pass": p}
			result.Push(datamap)
...
func SshScan(ScanType string, Target string, result mode.Result) {
	if port.PortCheck(Target, 22, result) {
